...
const withRegistryFixture = Effect.fn("EffectBoundaryRegistryTest.withFixture")(
  function* <A>(
    ...
    const root = yield* makeTempDirectoryScoped("expand-effect-registry-")
...
describe("Effect host-boundary registry", () => {
  ...
  it.live("accepts one exact tracked and consumed boundary", () =>
    fixture([boundary()], (input) => validateHostBoundaries(input)))

  it.live("resolves every permanent boundary against tracked AST and ESLint consumers", () =>
    Effect.gen(function*() {
      const path = yield* Path.Path
      ...
      const tracked = trackedReport.stdout.split("\0").filter(Boolean)
      const eslint = yield* runCommand("eslint", ["--config", "eslint.effect.config.mjs", ".", "--format", "json"])
      expect([0, 1]).toContain(eslint.exitCode)
      const eslintFiles = (yield* Schema.decodeUnknownEffect(EslintJson)(eslint.stdout)).map((result) =>
  ...
  it.live("rejects records whose file is missing from the tracked manifest", () =>
    expectInvalid([boundary({ file: "src/missing.ts" })]))

  it.live("rejects duplicate records", () =>
    expectInvalid([boundary(), boundary()]))

  it.live("rejects multiple records matching one source occurrence", () =>
    expectInvalid([boundary({ host: "First host" }), boundary({ host: "Second host" })]))

  it.live("rejects unconsumed occurrence identities", () =>
    expectInvalid([boundary({ occurrence: 1 })]))

  it.live("rejects a stale process exit boundary after the source call is removed", () =>
    expectInvalid([boundary({ construct: platformProcessExit })]))

  it.live("rejects boundaries without an ESLint consumer", () =>
