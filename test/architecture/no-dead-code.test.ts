...
//
// To reproduce locally: `bun run knip`. If this fails, either delete the dead
// code, un-export an over-exported value, or — only for a genuine false positive —
// justify it explicitly in knip.jsonc.
const repoRoot = join(dirname(fileURLToPath(import.meta.url)), "..", "..")

describe("no dead code", () => {
  it("knip finds no unused files, exports, types, or dependencies", () => {
    const result = spawnSync("bunx", ["knip-bun", "--no-progress"], {
      cwd: repoRoot,
      encoding: "utf8"
    })
