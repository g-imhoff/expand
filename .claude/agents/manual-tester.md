...
```bash
node --import tsx --input-type=module -e 'import { NodeServices } from "@effect/platform-node"; import { Effect, Layer } from "effect"; import { withClient } from "@expand/client-ts"; import { makeNodeAdapter } from "@expand/client-ts/adapters/node"; import { AppContext, makeAppContext } from "@expand/contracts/app-context"; const appContext=Layer.succeed(AppContext, makeAppContext(process.argv[1])); const adapter=makeNodeAdapter({backendCommand:[process.argv[2]]}); const hold=withClient(adapter, () => Effect.gen(function*(){ yield* Effect.sync(()=>console.log("open")); yield* Effect.never })); await Effect.runPromise(hold.pipe(Effect.provide(NodeServices.layer), Effect.provide(appContext)))' "$DATA_DIR" "$REPO_ROOT/dist/expand-server" >"$DATA_DIR/standing-client.log" 2>&1 &
```
