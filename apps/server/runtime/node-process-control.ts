...
export const ProcessServices = (() => {
  ...
  const currentIdentity = Effect.fn("ServerNodeProcessControl.currentIdentity")(function*() {
    return readIncarnation(process.pid)
  })
  const identify = Effect.fn("ServerNodeProcessControl.identify")(function*(pid: number) {
    return yield* Effect.try({
      try: () => process.kill(pid, 0),
      catch: (cause) => new ProcessProbeError({ pid, cause })
    }).pipe(
