...
describe("renderer feature folder layout", () => {
  ...
  it("colocates projects under components/model/pages/data", () => {
    ...
    expect(existsSync(at("apps/desktop/src/renderer/features/projects/data/project-context.tsx"))).toBe(true)
  })
  ...
  it("keeps the RPC layer under renderer/rpc with the renderer-port owner", () => {
    ...
    expect(existsSync(at("apps/desktop/src/renderer/rpc/server-rpc.ts"))).toBe(false)
  })
  ...
  it("leaves no files at the old flat command/ or projects/ locations and no removed modules", () => {
    ...
    expect(existsSync(at("apps/desktop/src/renderer/app/app-handle.ts"))).toBe(false)
    expect(existsSync(at("apps/desktop/src/renderer/app/AppHandleProvider.tsx"))).toBe(false)
  })
})
