...
A guided reading path for reviewing this branch. It is large — **324 commits, 260 files, ~16,700 insertions and only 35 deletions** — so treat everything here as *newly built*, not as a small diff on top of `develop`.
...
2. `db/event-store.ts` — the append-only `events` table; `append()` returns the new `seq`; `scan(options?)` is the ONE read primitive: a keyset-paginated Effect `Stream` (chunked `WHERE seq > ? ORDER BY seq LIMIT ?` reads — bounded memory, short WAL-friendly read transactions) with an optional `eventTypes` filter; an undecodable row is a **defect** carrying `{seq, stream_id, event_type}` (fail-fast — see the ADR; the old skip-with-warning tolerance is gone). Then `db/project-event-store.ts` — the first specialized store: `ProjectEventStore.read(fromSeq)`, a domain-named facade whose tag filter derives mechanically from the `ProjectEvent` union.
3. `domain/project.ts` — `projectsFromEvents`, the pure fold (delegates to the contracts statics).
4. `db/projection-state-store.ts` + `application/projections.ts` — the read-model cache. `projection-state-store.ts` is the name-keyed `projection_state {name, state, last_seq, fold_version}` table (`load` returns `null` on absent/NULL-state; `save` upserts state+cursor in ONE row — the persisted mirror of the C2 pair). `projections.ts` boot-catches-up via streamed folds (`ProjectEventStore.read`), then keeps checkpointing: a scoped debounce fiber (500ms quiet) plus a shutdown finalizer registered BEFORE the fiber so teardown interrupts-then-writes. `list`/`snapshot` read the `SubscriptionRef`; `apply` advances it with the C2 seq-gate.
...
5. `adapters/bun.ts` + `adapters/node.ts` — the two platform implementations (socket + spawn).
6. `project-store.ts` — **the core engine:** the session loop, the **C2 atomic fold**, the public mirror, the reconnect loop, and the mutation methods.
7. `supervise.ts` — logs a background fiber's death unless it was a clean interrupt.
8. `project-client.ts` + `client-layer.ts` — the stateless facades and how layers share one connection.
...
**Best tests to read:** `test/integration/snapshot-consistency.test.ts` (the C2 proof — 40 concurrent reads), `test/integration/bootstrap-window.test.ts`, `test/integration/reconnect.test.ts`, `test/integration/cross-store-sync.test.ts`.
