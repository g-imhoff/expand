...
DONE (2026-07-13) 4. `discovery.ts` + `spawn.ts` + `spawn-lock.ts` — `discovery.ts` validates the selected root's endpoint and owning PID; `spawn.ts` owns find-or-spawn and the **10-second** endpoint-advertisement deadline; `spawn-lock.ts` owns atomic lease publication plus token/inode-safe dead-owner recovery and release. Same-root callers converge, while distinct roots remain independent.
DONE 5. `rpc-client.ts` — `acquireClient`: builds the protocol layer, the presence handshake, stale-endpoint self-healing retry.
DONE 6. `adapters/bun.ts` + `adapters/node.ts` — the two platform implementations (socket + spawn); the platform subpath entrypoints (public alongside `/project` and `/server`).
