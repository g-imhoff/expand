...
| **I-3** | One discovery file per state root | Each root has one endpoint file (`url`, `token`, `pid`, `protocolVersion`); clients coordinate spawning through that root's `server.json.lock`. |
...
- **One explicit state root.** `AppContext` normalizes one data directory to an absolute path and derives the database, endpoint, and log paths from it. The CLI exposes it as a true global `--data-dir` flag; the desktop and standalone server retain raw-argv support; every spawned backend receives the same selected directory; and `backend.lock` prevents two live backends from sharing it.
...
NEW (2026-07-12) 10. `state-root-lock.ts` — the scoped `backend.lock` lease. Raw acquisition still rejects a live same-root owner immediately; startup rejects an advertised live owner but gives an endpoint-absent owner a bounded shutdown handoff, acquiring only after the prior lease is released. Dead-owner reclaim requires unchanged PID/token/inode evidence, invalid or changing evidence fails closed without retry, and release removes only its own token.
UPDATED (2026-07-12) 11. `composition/app.ts` → `main.ts` — lifecycle orchestration and the thin entrypoint. `main.ts` now performs default-home migration before the startup-aware `backend.lock` acquisition, then holds the exclusive lifetime lease **before the file logger**, database, or `AppLayer` is built. This ordering lets a replacement wait for an endpoint-absent predecessor to finish shutdown without overlapping ownership; logger creation also cannot pre-create the nested target and suppress migration. It sets `process.umask(0o077)` before anything touches the filesystem, while `app.ts` fail-closed-`chmod`s the data dir (`0700`), SQLite log (`0600`), and WAL/SHM sidecars if present (`secureIfPresent`).
...
UPDATED (2026-07-12) 4. `discovery.ts` + `spawn.ts` — `discovery.ts` validates the selected root's endpoint and owning PID; `spawn.ts` owns per-root find-or-spawn, the `server.json.lock` `O_EXCL` coordination dance, stale-lock recovery, and the increased **10-second** endpoint-advertisement deadline.
...
- **Per-root spawn convergence** (`spawn.ts`): the dead-pid/30s stale-`server.json.lock` heuristic and `ensuring(releaseLock)` prevent a crashed spawner from wedging future clients. Concurrent callers on one root must spawn once; callers on distinct roots must proceed independently. This lock coordinates clients; the server's `backend.lock` enforces lifetime uniqueness.
...
3. **`contracts/app-context.ts` → CLI `main.ts` → client `spawn.ts` → server migration → `state-root-lock.ts` → `binary-smoke.sh`** — trace one explicit data directory end to end, including legacy-home safety, per-root spawn coordination, and runtime ownership (45–60 min).
