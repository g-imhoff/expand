...
Code pin: `47395dae796a469240c766dcfd6a004a65a10a32`.
...
array. It must call the exported `usePromptImageUrlOwnership` hook with every
attachment it retains, including images in saved send payloads. The hook keeps
generated URLs valid while those references remain and revokes them when the
parent drops the last reference or unmounts. It leaves URLs that the prompt
image URL helper did not generate alone. The composer releases a generated
URL on unmount if the parent never accepted it. A remove button requests a
change through `onImagesChange`. Each attachment has a thumbnail, a remove
...
status announces sending. The picker and menus use their shared keyboard and
focus behavior.
...
These are full SHA-256 content hashes for the files at code pin `47395dae796a469240c766dcfd6a004a65a10a32`:

```text
e90a1b005c68409043ec9a0481b8b63a10ba06bc3c9afb70bce4743b555c4c0d  apps/desktop/src/renderer/features/chat/components/PromptInput.tsx
...
e42586d2eb4809d1b7351bfda955e8ecc7983d8cc21c9f5181ff9e99732caf83  apps/desktop/src/renderer/features/chat/components/prompt-image-urls.ts
...
10feb2a69aee55a90f6a949120a9d87906f736b19de307fb47503b94b44a774f  apps/desktop/test/ui/prompt-input.test.tsx
...
- `npm test -- apps/desktop/test/ui/prompt-input.test.tsx apps/desktop/test/ui/prompt-mentions.test.tsx`:
  48 tests passed across 2 files.
...
- `vite build examples/prompt-input-specimen --config examples/prompt-input-specimen/vite.config.ts`:
  passed. The local build has 3 files and 371946 bytes.
...
state for the menu options and images, and a send handler. It must call
`usePromptImageUrlOwnership` with all live images and every saved payload image
until it no longer needs those URLs. The host still decides when to clear the
images and must release any object URLs it created outside the composer. Full
page overflow, accessibility, and contrast checks belong with the conversation
host because the present homepage mount uses fixture data.
